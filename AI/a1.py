...
class Node:
    ...
    def __init__(self, checkerboard, ope, depth, alpha, beta, force_score, player_first):
        """
        创建一个minimax的节点
        ...
        """
        self.g_map = checkerboard  # 棋盘  [[0 for _ in range(15)] for _ in range(15)]  # 当前的棋盘
        self.ope = ope
        self.depth = depth
        self.alpha = alpha
        self.beta = beta
    ...
    def calc_score(self):
        ...
        for x in range(11):
            for y in range(15):
                player_cnt = sum(
                    [self.g_map[x][y] == 1, self.g_map[x + 1][y] == 1, self.g_map[x + 2][y] == 1,
                     self.g_map[x + 3][y] == 1, self.g_map[x + 4][y] == 1])
                ai_cnt = sum(
                    [self.g_map[x][y] == 2, self.g_map[x + 1][y] == 2, self.g_map[x + 2][y] == 2,
                     self.g_map[x + 3][y] == 2, self.g_map[x + 4][y] == 2])
        ...
        for x in range(15):
            for y in range(11):
                player_cnt = sum(
                    [self.g_map[x][y] == 1, self.g_map[x][y + 1] == 1, self.g_map[x][y + 2] == 1,
                     self.g_map[x][y + 3] == 1, self.g_map[x][y + 4] == 1])
                ai_cnt = sum(
                    [self.g_map[x][y] == 2, self.g_map[x][y + 1] == 2, self.g_map[x][y + 2] == 2,
                     self.g_map[x][y + 3] == 2, self.g_map[x][y + 4] == 2])
                if player_cnt == 4 and ai_cnt == 0:
                    player_4_num += 1
                if ai_cnt == 4 and player_cnt == 0:
                    ai_4_num += 1
        for x in range(11):
            for y in range(11):
                player_cnt = sum(
                    [self.g_map[x][y] == 1, self.g_map[x + 1][y + 1] == 1, self.g_map[x + 2][y + 2] == 1,
                     self.g_map[x + 3][y + 3] == 1, self.g_map[x + 4][y + 4] == 1])
                ai_cnt = sum(
                    [self.g_map[x][y] == 2, self.g_map[x + 1][y + 1] == 2, self.g_map[x + 2][y + 2] == 2,
                     self.g_map[x + 3][y + 3] == 2, self.g_map[x + 4][y + 4] == 2])
                if player_cnt == 4 and ai_cnt == 0:
                    player_4_num += 1
                if ai_cnt == 4 and player_cnt == 0:
                    ai_4_num += 1
        for x in range(11):
            for y in range(11):
                player_cnt = sum([self.g_map[x + 4][y] == 1, self.g_map[x + 3][y + 1] == 1,
                                  self.g_map[x + 2][y + 2] == 1, self.g_map[x + 1][y + 3] == 1,
                                  self.g_map[x][y + 4] == 1])
                ai_cnt = sum([self.g_map[x + 4][y] == 2, self.g_map[x + 3][y + 1] == 2,
                              self.g_map[x + 2][y + 2] == 2, self.g_map[x + 1][y + 3] == 2,
                              self.g_map[x][y + 4] == 2])
                if player_cnt == 4 and ai_cnt == 0:
                    player_4_num += 1
                if ai_cnt == 4 and player_cnt == 0:
        ...
        for x in range(11):
            for y in range(15):
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y] == 1 and self.g_map[x + 2][y] == 1 and
                    ...
                    player_3d_num += 1
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y] == 2 and self.g_map[x + 2][y] == 2 and
                        self.g_map[x + 3][y] == 2 and self.g_map[x + 4][y] == 0):
                    ai_3d_num += 1
        for x in range(15):
            for y in range(11):
                if (self.g_map[x][y] == 0 and self.g_map[x][y + 1] == 1 and self.g_map[x][y + 2] == 1 and
                    ...
                    player_3d_num += 1
                if (self.g_map[x][y] == 0 and self.g_map[x][y + 1] == 2 and self.g_map[x][y + 2] == 2 and
                        self.g_map[x][y + 3] == 2 and self.g_map[x][y + 4] == 0):
                    ai_3d_num += 1
        for x in range(11):
            for y in range(11):
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y + 1] == 1 and
                        self.g_map[x + 2][y + 2] == 1 and self.g_map[x + 3][y + 3] == 1 and
                        self.g_map[x + 4][y + 4] == 0):
                    player_3d_num += 1
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y + 1] == 2 and
                        self.g_map[x + 2][y + 2] == 2 and self.g_map[x + 3][y + 3] == 2 and
                        self.g_map[x + 4][y + 4] == 0):
                    ai_3d_num += 1
        for x in range(11):
            for y in range(11):
                if (self.g_map[x + 4][y] == 0 and self.g_map[x + 3][y + 1] == 1 and
                        self.g_map[x + 2][y + 2] == 1 and self.g_map[x + 1][y + 3] == 1 and
                        self.g_map[x][y + 4] == 0):
                    player_3d_num += 1
                if (self.g_map[x + 4][y] == 0 and self.g_map[x + 3][y + 1] == 2 and
                        self.g_map[x + 2][y + 2] == 2 and self.g_map[x + 1][y + 3] == 2 and
                        self.g_map[x][y + 4] == 0):
                    ai_3d_num += 1
        ...
        for x in range(10):
            for y in range(15):
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y] == 1 and (
                        (self.g_map[x + 2][y] == 1) ^ (self.g_map[x + 3][y] == 1)) and
                        self.g_map[x + 4][y] == 1 and self.g_map[x + 5][y] == 0):
                    player_3d_num += 1
                if (self.g_map[x][y] == 0 and self.g_map[x + 1][y] == 2 and
                        ((self.g_map[x + 2][y] == 2) ^ (self.g_map[x + 3][y] == 2)) and
                        self.g_map[x + 4][y] == 2 and self.g_map[x + 5][y] == 0):
                    ai_3d_num += 1
        for x in range(15):
            for y in range(10):
                if (self.g_map[x][y] == 0 and self.g_map[x][y + 1] == 1 and (
                        (self.g_map[x][y + 2] == 1) ^ (self.g_map[x][y + 3] == 1)) and
                        self.g_map[x][y + 4] == 1 and self.g_map[x][y + 5] == 0):
                    player_3d_num += 1
                if (self.g_map[x][y] == 0 and self.g_map[x][y + 1] == 2 and (
                        (self.g_map[x][y + 2] == 2) ^ (self.g_map[x][y + 3] == 2)) and
                        self.g_map[x][y + 4] == 2 and self.g_map[x][y + 5] == 0):
                    ai_3d_num += 1
        for x in range(10):
            for y in range(10):
                if self.g_map[x][y] == 0 and self.g_map[x + 1][y + 1] == 1 and (
                    ...
                    player_3d_num += 1
                if self.g_map[x][y] == 0 and self.g_map[x + 1][y + 1] == 2 and (
                        (self.g_map[x + 2][y + 2] == 2) ^ (self.g_map[x + 3][y + 3] == 2)) and \
                        self.g_map[x + 4][y + 4] == 2 and self.g_map[x + 5][y + 5] == 0:
                    ai_3d_num += 1
        for x in range(10):
            for y in range(10):
                if self.g_map[x + 5][y] == 0 and self.g_map[x + 4][y + 1] == 1 and (
                    ...
                    player_3d_num += 1
                if self.g_map[x + 5][y] == 0 and self.g_map[x + 4][y + 1] == 2 and (
                        (self.g_map[x + 3][y + 2] == 2) ^ (self.g_map[x + 2][y + 3] == 2)) and \
                        self.g_map[x + 1][y + 4] == 2 and self.g_map[x][y + 5] == 0:
                    ai_3d_num += 1
                ...
                if self.g_map[x][y] == 1:
                    ...
                    for x0 in range(x - 1, x + 1):
                        for y0 in range(y - 1, y + 1):
                            if 14 >= x0 >= 0 != self.g_map[x0][y0] and 0 <= y0 <= 14:
                                around_cnt += 1
...
class AI1Step:
    ...
    def __init__(self, checkerboard, init_depth, player_first: bool):
        """
        决定AI这一步走什么地方
        :param checkerboard: 棋盘
        ...
        """
        node_init = Node(copy.deepcopy(checkerboard), None, np.count_nonzero(np.array(init_depth)), -np.inf, np.inf,
                         False, player_first)  # 根节点
    ...
    def search(self, cur_node_dx, ope_hist, max_depth):
        ...
        for x in range(15):
            for y in range(15):
                if self.method_tree[cur_node_dx].g_map[x][y] != 0:
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 14 and 0 <= y0 <= 14 and (x0, y0) not in ope_list:
                                if self.method_tree[cur_node_dx].g_map[x0][y0] == 0:
                                    ope_list.add((x0, y0))
        # 2. 然后对每一个可以落子的格子进行搜索
        for cell in ope_list:
            # 2.1 创建一个子节点，并计算这个子节点的分数
            i_game = copy.deepcopy(self.method_tree[cur_node_dx].g_map)
            if self.player_first:
                if self.method_tree[cur_node_dx].depth % 2 == 0:  # 轮到玩家出
                    i_game[cell[0]][cell[1]] = 1
                else:  # 轮到电脑出
                    i_game[cell[0]][cell[1]] = 2
            else:
...
class Computer:
    ...
    def __init__(self):
        self.max_search_steps = 2  # 最远看2回合之后
        self.g_map = None
        self.ai = None

    def start(self, g_map: np.ndarray, player, index):
        """
        开始
        @return:
        """
        self.player = player
        self.index = index
        self.g_map = np.array(g_map)
        if not np.count_nonzero(self.g_map):
            return self.g_map.shape[0] // 2, self.g_map.shape[1] // 2
        self.ai = AI1Step(self.g_map, np.count_nonzero(np.array(self.g_map)), True)
        self.ai.search(0, [set(), set()], self.max_search_steps)
        if self.ai.next_node_dx_list[0] == -1:
            raise ValueError('ai.next_node_dx_list[0] == -1')
        ai_ope = self.ai.method_tree[self.ai.next_node_dx_list[0]].ope
